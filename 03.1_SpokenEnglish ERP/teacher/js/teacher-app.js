...
document.addEventListener('alpine:init', () => {
    Alpine.data('teacherApp', () => ({
        ...
        // Initialization
        init() {
            // Protect Route (Disabled for UI Task)
        ...
        async loadMatrixData() {
            ...
            try {
                ...
                if (gridContainer) gridContainer.innerHTML = `<div style="padding: 20px; color: var(--danger); text-align: center;">Network error loading matrix.</div>`;
            }
        },
    ...
    if (themeToggleBtn) {
        themeToggleBtn.addEventListener('click', () => {
            ...
            if (isDark) {
                ...
                localStorage.setItem('scs_theme', 'light');
            } else {
                document.body.parentElement.setAttribute('data-theme', 'dark');
                themeToggleBtn.textContent = '🌙';
                localStorage.setItem('scs_theme', 'dark');
            }
        });

        // Check on load
        if (localStorage.getItem('scs_theme') === 'dark') {
            document.body.parentElement.setAttribute('data-theme', 'dark');
            themeToggleBtn.textContent = '🌙';
        }
    }
